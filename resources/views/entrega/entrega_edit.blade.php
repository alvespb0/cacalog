...
<form action="{{route('update.entrega', $entrega->id)}}" method="POST">
    @csrf
    <div class="form-group">
        <label for="conteudo_entrega">Conteudo</label>
        <input type="text" class="form-control" name="conteudo_entrega" required value = "{{$entrega->conteudo_entrega}}">
    </div>
    <div class="form-group">
        <label for="codigo_pedido">Codigo do Pedido</label>
        <input type="number" class="form-control" name="codigo_pedido" required min="0" value = "{{$entrega->codigo_pedido}}">
    </div>
    <div class="form-group">
        ...
        <select name="motoboy_id" class="form-select"  id="" required>
            ...
            <option value="{{$motoboy->id}}" {{$entrega->motoboy_id == $motoboy->id ? 'selected' : ''}}>{{$motoboy->name}}</option>
            @endforeach
        </select>        
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
        ...
        <select name="endereco_id" class ="form-control" id="">
            ...
            <option value="{{$endereco->id}}" {{$entrega->endereco_id == $endereco->id ? 'selected' : ''}}>{{$endereco->logradouro}}</option>
            @endforeach
        </select>
    </div>
    <div class="form-group">
        ...
        <select name="status_id" class ="form-control" id="">
            ...
            @foreach($status as $s)
            <option value="{{$s->id}}" {{$entrega->status_id == $s->id ? 'selected' : ''}}>{{$s->nome}}</option>
            @endforeach
        </select>
    </div>
    <br>
