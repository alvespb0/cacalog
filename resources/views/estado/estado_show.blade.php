        ...
        <table class="table table-hover table-bordered text-center align-middle" style="border-radius: 10px; overflow: hidden;">
            ...
            @foreach($estados as $estado)
                    <tr>
                        ...
                        <td>{{ $estado->name }}</td>
                        <td> 
                            <a href="{{ route('alteracao.estado', ['id' => $estado->id]) }}" class="btn btn-outline-primary btn-sm me-2">
                                <i class="bi bi-pencil-square"></i> Editar
                            </a>
                            <a href="{{ route('delete.estado', ['id' => $estado->id])}}" class="btn btn-outline-danger btn-sm" onclick="return confirm('Tem certeza que deseja excluir?')">
